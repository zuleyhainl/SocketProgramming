...
#define messageSize 512//max message size 512byte
typedef char message[messageSize];
...
int main() {
	...
	char message[messageSize], command[550];//command holds both command and message for send
	...
	while(1)
	{
		...
		if(strcmp(command, "SEND") == 0 && threadExistFlag == 0)//send message over socket
		{
			...
			if(token != NULL)
			{
				...
				puts(message);
				send(sock_fd, message, messageSize, 0);	
			}
		}
		...
		else if(strcmp(command, "QUIT") == 0 && threadExistFlag == 0)
		{
			...
			send(sock_fd, command, messageSize, 0);
...
void* auto_handler(void* passingValues)//auto thread func
{
    ...
    while(1)
	{
		...
		if(noNewMessage(*passing, passing->multicast_queue) == -1 || isEmptyBucket(passing->multicast_queue) == -1)
		{
			//puts("No new messages. Blocking...\n");
...
{
    if(multicast_queue->head == multicast_queue->tail && multicast_queue->isFull == 0)
	{
		return -1;
	}
    return 0;
